...
3. 每天按设定时刻自动执行（默认 00:01，03:01 失败兜底；两个时刻都可在插件设置里改，见下文「自定义执行时刻」）
...
| `cronTime`（选择） | `1 0 * * *`（00:01） | 每日签到时刻；选项 00:01 / 00:30 / 01:30 / 07:30 / 12:30 |
| `retryCron`（选择） | `1 3 * * *`（03:01） | 失败重试时刻；选项 03:01 / 03:30 / 04:30 / 10:30 / 15:30 |
