...
- 预期成功：`Sign: ok | Share: ok | link=...`
- **分享机制说明**：脚本完成"分享动作"（两步法：生成分享码 + 上报），即触发 +5 积分，加分为异步落账。复查到加分时显示 `Share: ok (+5 已到账)`，否则显示 `Share: ok`（可在 App 积分中确认到账）。
...
| `Share: ok (+5 已到账)` | 分享动作完成且积分复查已确认 +5 到账 | 无需处理 |
| `Share: ok` | 分享动作完成（两步法即触发加分，加分为异步落账） | 无需处理，可在 App 积分中查看 |
...
**没有收到任何通知 / 定时未执行**
- 确认插件已启用（Loon → 插件列表 → 开关）、脚本 URL 带最新版本号（`v=...`）
- 定时依赖设备在线：00:01 时手机需联网且 Loon 未被系统杀掉（iOS 可把 Loon 设为"始终允许后台活动"，避免锁屏后被清理）
- 脚本托管在 `raw.githubusercontent.com`，网络不畅时 Loon 可能拉不到脚本而跳过执行；可换用镜像加速（如 `ghproxy.com/https://raw.githubusercontent.com/...`）或稍后手动执行
- 每次执行超时上限 120 秒

**每日结果通知的弹窗次数**
- 每天最多 1 条结果通知（00:01）；03:01 兜底发现"今日已完成"时静默跳过，不再弹窗
